...
// Components:
import PasswordBooleanInput from "./PasswordBooleanInput";
...
import PasswordStyledInput from "./PasswordStyledInput";
...
interface PasswordGeneratorPanelProps {
  onNewPassword?:
    | ((newPassword: string) => void | never)
    | ((newPassword: string, passwordOptions: PasswordOptions) => void | never);
}

const PasswordGeneratorPanel: FC<PasswordGeneratorPanelProps> = ({
  onNewPassword,
}) => {
  // State:
  ...
  function handleGeneratePassword(): void {
    ...
    if (onNewPassword && newPassword) {
      onNewPassword(newPassword, passwordOptions);
    }
  }
